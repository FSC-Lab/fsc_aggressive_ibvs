...
public:
  ...
  void initialize(const vision_common::StateEstimate& state_estimate);
  void updateWithStateEstimate(
      const vision_common::StateEstimate& state_estimate);
  vision_common::StateEstimate predictState(const ros::Time& time) const;
  bool isInitialized();

  void pushCommandToQueue(const vision_common::ControlCommand& cmd);
