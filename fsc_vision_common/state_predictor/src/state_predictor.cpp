...
// #include <vision_common/quaternion_functions.h>
    ...
    const vision_common::StateEstimate& state_estimate)
{
  ...
  if (!command_queue_.empty())
  {
    ...
    {
      std::list<vision_common::ControlCommand>::reverse_iterator
...
vision_common::StateEstimate StatePredictor::predictState(
    ...
    const ros::Time& time) const
{
      ...
      < command_queue_.back().expected_execution_time)
  {
    ...
    vision_common::limit(&command_execution_time, 0.0, time_left_to_predict);
      ...
      command_it != command_queue_.rend(); command_it++)
  {
    ...
    if (next_command != command_queue_.rend())
    {
      ...
      double command_execution_time = (next_command->expected_execution_time
          - predicted_ext_state.timestamp).toSec();
      vision_common::limit(&command_execution_time, 0.0,
...
void StatePredictor::pushCommandToQueue(
    const vision_common::ControlCommand& cmd)
{
...
QuadExtStateEstimate StatePredictor::dynamics(
    const QuadExtStateEstimate& old_state,
    const vision_common::ControlCommand& cmd, const double dt) const
{
  ...
  else
  {
    ...
    new_state.orientation = vision_common::integrateQuaternion(
...
{
  vision_common::getParam("state_predictor/integration_step_size",
                             integration_step_size_, 0.001, pnh_);
  vision_common::getParam("state_predictor/maximum_prediction_horizon",
                             maximum_prediction_horizon_, 0.1, pnh_);

  if (!vision_common::getParam("state_predictor/tau_thrust_up_dynamics",
                                  tau_thrust_up_dynamics_, pnh_))
  {
    return false;
  }
  if (!vision_common::getParam("state_predictor/tau_thrust_down_dynamics",
                                  tau_thrust_down_dynamics_, pnh_))
  {
    return false;
  }
  if (!vision_common::getParam("state_predictor/tau_angle_rate_dynamics",
