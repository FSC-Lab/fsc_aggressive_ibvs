...
Trajectory::Trajectory(const vision_msgs::Trajectory& trajectory_msg) {
  timestamp = trajectory_msg.header.stamp;

  switch (trajectory_msg.type) {
    case vision_msgs::Trajectory::GENERAL:
      trajectory_type = TrajectoryType::GENERAL;
      break;
    case vision_msgs::Trajectory::ACCELERATION:
      trajectory_type = TrajectoryType::ACCELERATION;
      break;
    case vision_msgs::Trajectory::JERK:
      trajectory_type = TrajectoryType::JERK;
      break;
    case vision_msgs::Trajectory::SNAP:
  ...
  for (int i = 0; i < trajectory_msg.points.size(); i++) {
    points.push_back(vision_common::TrajectoryPoint(trajectory_msg.points[i]));
  }
}

Trajectory::Trajectory(const vision_common::TrajectoryPoint& point)
    : timestamp(ros::Time::now()),
      trajectory_type(TrajectoryType::GENERAL),
      points() {
...
vision_msgs::Trajectory Trajectory::toRosMessage() const {
  vision_msgs::Trajectory ros_msg;
...
vision_common::TrajectoryPoint Trajectory::getStateAtTime(
  ...
  std::list<vision_common::TrajectoryPoint>::const_iterator p0 = std::prev(
    p1);
