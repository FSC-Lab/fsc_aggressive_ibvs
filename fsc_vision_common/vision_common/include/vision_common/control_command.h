...
struct ControlCommand {
  ...
  ControlCommand();
  ControlCommand(const vision_msgs::ControlCommand& control_command_msg);
  virtual ~ControlCommand();

  void zero();
  vision_msgs::ControlCommand toRosMessage() const;
  ...
  std::vector<double> slack_variables;  // [m/s^2]
