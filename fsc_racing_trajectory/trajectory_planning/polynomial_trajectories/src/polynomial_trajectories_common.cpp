...
vision_common::TrajectoryPoint getPointFromTrajectory(
    const PolynomialTrajectory& trajectory,
    const ros::Duration& time_from_start) {
  vision_common::TrajectoryPoint desired_state;
...
void computeMaxima(const PolynomialTrajectory& trajectory,
  ...
  *maximal_snap = 0.0;
  ...
  while (t + dt <= trajectory.T.toSec()) {
    t += dt;
    vision_common::TrajectoryPoint state =
...
double computeRollPitchRateNormFromTrajectoryPoint(
    const vision_common::TrajectoryPoint& desired_state) {
  const Eigen::Vector3d gravity(0.0, 0.0, 9.81);
