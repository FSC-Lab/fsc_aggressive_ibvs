#include "fsc_rotors_interface/fsc_rotors_interface.h"
...
namespace fsc_rotors_interface {
...
RPGRotorsInterface::RPGRotorsInterface(const ros::NodeHandle& nh,
  ...
  rpg_control_command_sub_ =
      nh_.subscribe("autopilot/control_command", 1,
  ...
  arm_interface_sub_ =
      nh_.subscribe("fsc_rotors_interface/arm", 1,
...
void RPGRotorsInterface::lowLevelControlLoop(const ros::TimerEvent& time) {
  ...
  const vision_common::ControlCommand rate_cmd =
      vision_common::ControlCommand(control_command_);

  const TorquesAndThrust torques_and_thrust = bodyRateControl(
      rate_cmd,
      vision_common::geometryToEigen(quad_state_.twist.twist.angular));
  ...
  //     const vision_common::ControlCommand rate_cmd = attitudeControl(
  //         vision_common::ControlCommand(control_command_),
  //         vision_common::geometryToEigen(quad_state_.pose.pose.orientation));
  ...
  //         inertia_ * vision_common::geometryToEigen(
  //                        control_command_.angular_accelerations) +
  //         vision_common::geometryToEigen(control_command_.bodyrates)
  //             .cross(inertia_ * vision_common::geometryToEigen(
  ...
  //       vision_common::limit(&motor_speed_squared, 0.0,
...
void RPGRotorsInterface::rpgControlCommandCallback(
    const vision_msgs::ControlCommand::ConstPtr& msg) {

  // std::cout << "rpgControlCommandCallback" << std::endl;
  control_command_ = *msg;
}

vision_common::ControlCommand RPGRotorsInterface::attitudeControl(
    const vision_common::ControlCommand& attitude_cmd,
    const Eigen::Quaterniond& attitude_estimate) {
  Eigen::Quaterniond q_e =
      attitude_estimate.inverse() * attitude_cmd.orientation;

  vision_common::ControlCommand body_rate_command = attitude_cmd;
  body_rate_command.control_mode = vision_common::ControlMode::BODY_RATES;
...
mav_msgs::Actuators RPGRotorsInterface::mixer(
  ...
  for (int i = 0; i < 4; i++) {
    vision_common::limit(&rotor_speed_cmds.angular_velocities[i], 0.0,
                            pow(max_rotor_speed_, 2.0));
...
void RPGRotorsInterface::loadParameters() {
  ...
  vision_common::getParam("inertia_x", inertia_(0, 0), 0.007, pnh_);
  vision_common::getParam("inertia_y", inertia_(1, 1), 0.007, pnh_);
  vision_common::getParam("inertia_z", inertia_(2, 2), 0.012, pnh_);
  ...
  vision_common::getParam("body_rates_p_xy", body_rates_p_xy, 0.15, pnh_);
  vision_common::getParam("body_rates_d_xy", body_rates_d_xy, 0.5, pnh_);
  vision_common::getParam("body_rates_p_z", body_rates_p_z, 0.03, pnh_);
  vision_common::getParam("body_rates_d_z", body_rates_d_z, 0.1, pnh_);
  ...
  vision_common::getParam("low_level_control_frequency",
                             low_level_control_frequency_, 200.0, pnh_);

  vision_common::getParam("roll_pitch_cont_gain", roll_pitch_cont_gain_, 6.0,
                             pnh_);

  vision_common::getParam("arm_length", arm_length_, 0.17, pnh_);

  vision_common::getParam("rotor_drag_coeff", rotor_drag_coeff_, 0.016,
                             pnh_);
  vision_common::getParam("rotor_thrust_coeff", rotor_thrust_coeff_,
                             8.54858e-06, pnh_);
  vision_common::getParam("mass", mass_, 0.68 + 0.009 * 4.0, pnh_);
  vision_common::getParam("max_rotor_speed", max_rotor_speed_, 838.0, pnh_);
}

}  // namespace fsc_rotors_interface

int main(int argc, char** argv) {
  ros::init(argc, argv, "fsc_rotors_interface");
  ...
  // Run the interface
  fsc_rotors_interface::RPGRotorsInterface fsc_rotors_interface;
  ros::spin();
